...
export default function IssueHeader({ issue, title, no }: IssueHeaderProps) {
  return (
    <div className="relative overflow-hidden border-b-4 border-gaip-black bg-gaip-black text-gaip-white">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-3 -top-3 h-8 w-8 rotate-45 bg-action-yellow"
      />

      <div className="mx-auto flex max-w-editorial items-center gap-4 px-6 py-3">
        {/* Left: the comic imprint. Hidden on the narrowest screens so the
            title never gets crowded. */}
        <span className="hidden shrink-0 font-mono text-[0.7rem] uppercase tracking-[0.16em] text-gaip-white/80 sm:inline">
          Get AI Powers Comics
        </span>

        {/* Centre: issue label + Zilla Slab title. */}
        <div className="flex min-w-0 flex-1 flex-col items-center text-center">
          <span className="font-mono text-[0.65rem] uppercase tracking-[0.18em] text-action-yellow">
            {issue}
          </span>
          <span className="truncate font-display text-sm font-bold uppercase tracking-wide text-gaip-white sm:text-base">
            {title}
          </span>
        </div>

        {/* Right: action-yellow narration chip, black ink text. */}
        <span className="shrink-0 -rotate-2 border-2 border-gaip-black bg-action-yellow px-2.5 py-1 font-mono text-[0.7rem] font-bold uppercase tracking-wide text-gaip-black">
          {no}
        </span>
      </div>
    </div>
