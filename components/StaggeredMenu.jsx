...
// React Bits StaggeredMenu - vendored with principal patches:
//  - next/link for menu items so the comic page transition still fires
//  - Escape closes the menu (parity with the old overlay)
//  - reduced-motion opens/closes instantly (gsap.set, no timelines)
//  - menu closes itself when an item is clicked
// Styling is brand-adapted in StaggeredMenu.css (flat, no backdrop blur).
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#FFC72C', '#00B0BE'];
          let arr = [...raw];
          if (arr.length >= 3) {
